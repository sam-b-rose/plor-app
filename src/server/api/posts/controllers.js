...
export const index = {
  async get(req, res) {
    try {
      ...
      if (!posts)
        throw new ServerError('No posts exist at this moment.', {
          status: 404
        });
      res.json(posts);
    } catch (error) {
...
export const email = {
  async get(req, res) {
    try {
      ...
      if (!posts)
        throw new ServerError(`No posts belonging to '${req.params.email}'.`, {
          status: 404
        });
      res.json(posts);
    } catch (error) {
    ...
    try {
      ...
      if (!fetchedPost)
        throw new ServerError(
          `Post with id '${req.params.postid}' doesn't exist.`,
          { status: 404 }
        );
      res.json({

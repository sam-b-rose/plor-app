...
export const index = {
  async get(req, res) {
    try {
      ...
      if (!users)
        throw new ServerError('No users exist at this moment.', {
          status: 404
        });
      res.json(users);
    } catch (error) {
  ...
  async post(req, res) {
    try {
      ...
      if (password1 === password2) {
        ...
        let token = jwt.sign(savedUser, process.env.SECRET, {
          expiresIn: '30 days',
          jwtid: randId()
        });
        res.status(200).json({
          message: `Welcome, ${savedUser.fullName}!`,
          token,
          savedUser
        });
      } else {
...
export const email = {
  async get(req, res) {
    try {
      // check if the logged in user has the same email as the requested user.
      if (req.user.email === req.params.email) {
        ...
        if (!fetchedUser)
          throw new ServerError(
            `User with email '${req.params.email}' doesn't exist.`,
            { status: 404 }
          );
        res.json({
          email: fetchedUser.email,
          message: `Authentication by ${
            req.params.fetchedUser.email
  ...
  async post(req, res) {
    try {
      ...
      if (!user || !matched || !email || !password) {
        ...
        res
          .status(200)
          .json({ message: `Welcome, ${user.fullName}!`, token, user });
      }
    } catch (error) {
      res.handleServerError(error);
    }
